...
class TouchProcessor {
	...
	@:noCompletion private var __lastTaps:Array<Touch>;
	...
	/** A Array of arrays with the arguments that were passed to the "enqueue"
	 * method (the oldest being at the end of the Array). */
	@:noCompletion private var __queue:Array<TouchData>;

	/** The list of all currently active touches. */
	@:noCompletion private var __currentTouches:Array<Touch>;
	...
	private static function __init__() {
		untyped global.Object.defineProperties(TouchProcessor.prototype, {
			...
			"multitapTime": {
				get: untyped __js__("function () { return this.get_multitapTime (); }"),
				set: untyped __js__("function (v) { return this.set_multitapTime (v); }")
			},
			"multitapDistance": {
				get: untyped __js__("function () { return this.get_multitapDistance (); }"),
				set: untyped __js__("function (v) { return this.set_multitapDistance (v); }")
			},
			...
			"occlusionTest": {
				get: untyped __js__("function () { return this.get_occlusionTest (); }"),
				set: untyped __js__("function (v) { return this.set_occlusionTest (v); }")
			},
	...
	public function new(stage:Stage) {
		...
		__elapsedTime = 0.0;
		__currentTouches = new Array<Touch>();
		__queue = new Array<TouchData>();
		__lastTaps = new Array<Touch>();
	...
	public function advanceTime(passedTime:Float):Void {
		...
		#if (haxe_ver >= 4.0)
		sUpdatedTouches.resize(0);
		#else
		ArrayUtil.resize(sUpdatedTouches, 0);
		#end
		...
		while (__queue.length > 0 || numIterations == 0) {
			...
			while (__queue.length > 0 && !containsTouchWithID(sUpdatedTouches, __queue[__queue.length - 1].id)) {
	 ...
	 */
	private function processTouches(touches:Array<Touch>, shiftDown:Bool, ctrlDown:Bool):Void {
		var touch:Touch;

		#if (haxe_ver >= 4.0)
		sHoveringTouchData.resize(0);
		#else
		ArrayUtil.resize(sHoveringTouchData, 0);
		#end
	...
	private function checkForSystemGesture(touchID:Int, phase:String, globalX:Float, globalY:Float):Bool {
		...
		} else if (phase == TouchPhase.BEGAN) // also: _systemGestureTouchID < 0
		{
			...
			isGesture = globalX < screenBounds.left + __systemGestureMargins[LEFT]
			...
			if (isGesture)
				__systemGestureTouchID = touchID;
			return isGesture;
		} else
			return false;
	}
	...
	public function enqueue(touchID:Int, phase:String, globalX:Float, globalY:Float, pressure:Float = 1.0, width:Float = 1.0, height:Float = 1.0):Void {
	...
	private function queue_unshift(touchID:Int, phase:String, globalX:Float, globalY:Float, pressure:Float = 1.0, width:Float = 1.0, height:Float = 1.0):Void {
	...
	public function enqueueMouseLeftStage():Void {
		...
		if (minDist == distLeft)
			...
			exitX = __stage.stageWidth + offset;
		else if (minDist == distTop)
			exitY = -offset;
		else
			exitY = __stage.stageHeight + offset;
	...
	public function cancelTouches():Void {
		...
		// purge touches
		#if (haxe_ver >= 4.0)
		__currentTouches.resize(0);
		#else
		ArrayUtil.resize(__currentTouches, 0);
		#end
	...
	private function createOrUpdateTouch(touchData:TouchData):Touch {
		...
		touch.update(__elapsedTime, touchData.phase, touchData.globalX, touchData.globalY, touchData.pressure, touchData.width, touchData.height);
	...
	private function updateTapCount(touch:Touch):Void {
		...
		for (tap in __lastTaps) {
			var sqDist:Float = Math.pow(tap.globalX - touch.globalX, 2) + Math.pow(tap.globalY - touch.globalY, 2);
			if (sqDist <= minSqDist) {
				nearbyTap = tap;
				break;
			}
		...
		if (nearbyTap != null) {
			...
			__lastTaps.splice(__lastTaps.indexOf(nearbyTap), 1);
		} else {
			touch.tapCount = 1;
		}
	 ...
	 */
	public function setSystemGestureMargins(topMargin:Float = 10, bottomMargin:Float = 10, leftMargin:Float = 0, rightMargin:Float = 0):Void {
	...
	private function get_multitapDistance():Float {
		return __multitapDistance;
	}

	private function set_multitapDistance(value:Float):Float {
		return __multitapDistance = value;
	}
	...
	private function get_numCurrentTouches():Int {
		return __currentTouches.length;
	}
	...
	private function set_occlusionTest(value:Float->Float->Bool):Float->Float->Bool {
		return __occlusionTest = value;
	}

	private function get_occlusionTest():Float->Float->Bool {
		return __occlusionTest;
	}
	...
	private function get_discardSystemGestures():Bool {
		return __discardSystemGestures;
	}
